...
import { addSidedish, editSidedish  , deleSidedish } from '../../../../../../../../actions/VendorActions';
import "../starterdish/Starter.css"
...
function Sidedish() {
  ...
  const editDish = (idx) => {
    const Item = menuList?.sidedish?.filter((item, index) => {
      if (index === idx) return item;
    })
    setItem(Item)
    setIndex(idx);
  ...
  const AddSidedish= () => {
    console.log(data);
    dispatch(addSidedish(data, email));
  ...
  const handleSaveChanges = () => {
    ...
    dispatch(editSidedish({data,index,email}));
    setEditeddata({ itemName: "", itemDescription: "", itemPrice: "" });
    setShow(false)
  }
  const delItem = ()=>{
    dispatch(deleSidedish({index,email,item}))
    setShow(false)
  }
  return (
    <>
    {
      ...
      { editData && (
        <Modal show={show} onHide={handleClose}>
          <Modal.Header closeButton>
            <Modal.Title>EDIT SIDE DISH</Modal.Title>
          </Modal.Header>
          <Modal.Body>
      ...
      <div className="StarterContainer">
        <div className="StarterWrapper">
          <div className="Starter">
            ...
            <button onClick={AddSidedish} className="sbtBtn">ADD SIDE DISH</button>
          </div>
        </div>
        ...
        <table>
          <thead>
            <tr>
              ...
              <th scope="col">Side Dish Name</th>
          ...
          <tbody>
            {
              menuList?.sidedish?.map((item, index) => (
                <tr key={index}>
                  <td >{index + 1}</td>
                  <td>{item.itemName}</td>
