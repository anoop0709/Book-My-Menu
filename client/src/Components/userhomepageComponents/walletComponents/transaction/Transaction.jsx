        ...
        <div className="bookingList">
          {wallet?.transactions.slice(0,visible).map((details, index) => (
            <div className="bookingsRow" key={index}>
              ...
              <li className= {details.transactionType == "credit" ? "bookingrowItemgreen" : "bookingrowItem"}>{details.transactionType}</li>
              </div>
          ))}
          {
            visible < wallet?.transactions.length && (
              <div style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"flex-end"}}>
                <button className='btnMore' onClick={loadMore}>Load More</button>
              </div>
            )
          }
              </div>
