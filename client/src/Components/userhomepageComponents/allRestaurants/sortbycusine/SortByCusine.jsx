...
function SortByCusine({setCusine,setLocation,setFilteredData}) {
   ...
   <div className="cusineBox">
       {
           cusineType.map((cusine,index)=>(
               <button key={index} className={selected === index ? "cusinebtn active" : "cusinebtn" } onClick={()=>{setCusine(cusineType[index]);setSelected(index);setLocation("");setFilteredData([])}}>{cusine}</button>
           ))
       }
