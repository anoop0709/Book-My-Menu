          ...
          {loading ? (
            ...
            <div className="availableSlots">
              {seatavailable && (
                <div className="error">
                  <p>
                    Seats not available for {dateobj.number} people at this
                    time-slot{" "}
                  </p>
                </div>
              )}
              <div className="availableSlotWrapper">
                {timeSlot?.map((tm, index) => (
                  <div
                    className={
                      slots?.restBooked[0]?.obj[tm] < restaurant.seatingcapacity
                        ? "slotbox-orange"
                        : "slotBox"
                    }
                    key={index}
                  ...
                  >
                    ...
                    <p>
                      Available seats:{" "}
                      {restaurant[0]?.seatingcapacity -
                        (slots?.restBooked.length
                          ? parseInt(slots?.restBooked[0]?.obj[tm])
                          : 0)}
                    </p>
                  </div>
                ))}
